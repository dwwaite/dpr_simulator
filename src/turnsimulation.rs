...
fn process_ac_iteration(mut attack_profile: AttackProfile, number_turns: i32) -> Result<DataFrame, Box<dyn Error>> {

    // Prepare the results containers - created and unmodified
    let iteration_counter: Vec<i32> = (0..number_turns)
        .map(|x| x + 1)
        .collect();
    let ac_record: Vec<i32> = vec![attack_profile.target_ac; number_turns as usize];

    // Prepare the results containers - vectors which require growth
    let mut crit_counter: Vec<i32> = Vec::new();
    let mut damage_counter: Vec<i32> = Vec::new();

    // Process for the given AC iteration
    ...
    let results_df = match df!(
        ...
        Ok(df) => df,
        _ => bail!(format!("Unable to produce results matrix for AC value '{}'!", attack_profile.target_ac))
    };
    Ok(results_df)
}
...
pub fn process_simulation(
    ...
    // Prepare a vector of all AttackProfile iterations to be tested.
    let profile_vector = bundle_inputs(ac_targets, to_hit, mainhand_attacks, mainhand_notation, offhand_attack, offhand_string);
...
mod tests {
    ...
    fn test_bundle_inputs_both() {
        ...
        let exp_values: Vec<AttackProfile> = vec![
            AttackProfile::new(1, 1, 1, 1, DamageElement::from_notation_string(&mh_string), DamageElement::from_notation_string(&oh_string)),
            AttackProfile::new(2, 1, 1, 1, DamageElement::from_notation_string(&mh_string), DamageElement::from_notation_string(&oh_string)),
        ];
    ...
    fn test_process_ac_iteration() {
        ...
        assert_eq!(&Series::new("Iteration", &[1, 2, 3, 4, 5]), obs_output.column("Iteration").unwrap());
        assert_eq!(&Series::new("Target_AC", &[1, 1, 1, 1, 1]), obs_output.column("Target_AC").unwrap());
    }
    ...
    fn test_gather_dataframes() {
        ...
        assert_eq!(&Series::new("Numbers", &[1, 2, 3, 4, 5, 6]), obs_output.column("Numbers").unwrap());
        assert_eq!(&Series::new("Letters", &["a", "b", "c", "d", "e", "f"]), obs_output.column("Letters").unwrap());
    }
    ...
    fn test_process_simulation() {
        ...
        */
        ...
        let obs_result = process_simulation(ac_targets, to_hit, mh_attacks, mh_notation, oh_attacks, oh_notation, number_turns);
        assert!(obs_result.is_ok());

        // Test the predictable output columns - expecting 3 (AC) repeats of 2 turns.
        let obs_output = obs_result.unwrap();
        assert_eq!((6, 4), obs_output.shape());
        assert_eq!(&Series::new("Iteration", &[1, 2, 1, 2, 1, 2]), obs_output.column("Iteration").unwrap());
        assert_eq!(&Series::new("Target_AC", &[1, 1, 2, 2, 3, 3]), obs_output.column("Target_AC").unwrap());
    }
